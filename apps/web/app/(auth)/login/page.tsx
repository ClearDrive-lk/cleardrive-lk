...
import { Loader2, Lock, ShieldCheck, Terminal, AlertCircle } from 'lucide-react';
    ...
    <div className="min-h-screen w-full flex bg-[#050505] relative overflow-hidden font-sans selection:bg-[#FE7743] selection:text-black">
      ...
      <div className="hidden lg:flex w-1/2 relative flex-col justify-between p-12 z-10 border-r border-white/5">
        ...
        <div className="relative z-10">
            ...
            <div className="mt-4 flex gap-2">
                <Badge variant="outline" className="border-[#FE7743]/20 text-[#FE7743] bg-[#FE7743]/5 flex items-center gap-1">
                    <span className="w-1.5 h-1.5 rounded-full bg-[#FE7743] animate-pulse" />
                    SYSTEM ONLINE
                </Badge>
                <Badge variant="outline" className="border-white/10 text-gray-500">v2.4.0-stable</Badge>
            </div>
        </div>
            ...
            <div className="space-y-4">
                <div className="flex items-start gap-4 p-4 rounded-lg bg-white/5 border border-white/10 backdrop-blur-md hover:bg-white/10 transition-colors">
                    ...
                    <div>
                        ...
                        <p className="text-sm text-gray-400 mt-1">Secure gateway to USS Tokyo & JAA live auction data with sub-millisecond latency.</p>
                    </div>
                </div>
                <div className="flex items-start gap-4 p-4 rounded-lg bg-white/5 border border-white/10 backdrop-blur-md hover:bg-white/10 transition-colors">
                    ...
                    <div>
                        ...
                        <p className="text-sm text-gray-400 mt-1">Financial data and bidding instructions are protected by AES-256 GCM encryption.</p>
                    </div>
                </div>
            </div>
        </div>
      ...
      <div className="w-full lg:w-1/2 flex items-center justify-center p-6 relative z-10">
        ...
        <div className="w-full max-w-md bg-[#0A0A0A] border border-white/10 p-8 rounded-2xl shadow-2xl relative z-10">
            <div className="mb-8">
                <h3 className="text-2xl font-bold text-white mb-2">Terminal Access</h3>
                <p className="text-gray-400 text-sm">Please identify yourself to proceed.</p>
            </div>

            <form onSubmit={handleEmailLogin} className="space-y-5">
                <div className="space-y-2">
                    <Label htmlFor="email" className="text-xs font-mono text-gray-400 uppercase">Agent ID / Email</Label>
                    <Input 
                        id="email" 
                ...
                <div className="space-y-2">
                    <div className="flex justify-between items-center">
                        <Label htmlFor="password" className="text-xs font-mono text-gray-400 uppercase">Access Key</Label>
                        <Link href="#" className="text-xs text-[#FE7743] hover:text-[#FE7743]/80 transition-colors">Lost Key?</Link>
                    </div>
                    <Input 
                        id="password" 
                        type="password" 
                ...
                >
                    {loading ? <Loader2 className="animate-spin" /> : 'Authenticate Session'}
                </Button>
            </form>

            <div className="relative my-8">
                <div className="absolute inset-0 flex items-center"><span className="w-full border-t border-white/10"></span></div>
                <div className="relative flex justify-center text-xs uppercase"><span className="bg-[#0A0A0A] px-2 text-gray-500 font-mono">Or connect with</span></div>
            </div>
            ...
            <div className="mt-8 pt-6 border-t border-white/5 text-center text-sm text-gray-500">
                New Dealer? <Link href="/register" className="text-white hover:text-[#FE7743] font-medium transition-colors">Request API Access</Link>
            </div>
        </div>
      </div>
    </div>

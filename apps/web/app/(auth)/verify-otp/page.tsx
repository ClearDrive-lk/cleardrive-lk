        ...
        <div className="min-h-screen w-full flex bg-[#050505] relative overflow-hidden font-sans selection:bg-[#FE7743] selection:text-black">
            <div className="hidden lg:flex w-1/2 relative flex-col justify-between p-12 z-10 border-r border-white/5">
                ...
                <div className="relative z-10">
                    ...
                    <Badge variant="outline" className="mt-2 border-green-900/50 text-green-500 bg-green-900/10">ENCRYPTED CONNECTION</Badge>
                </div>
                ...
                <div className="relative z-10 space-y-6 max-w-lg">
                    ...
                    <div className="space-y-4">
                        <div className="flex items-center gap-3 text-gray-400 group">
                            <div className="w-10 h-10 rounded-full bg-white/5 flex items-center justify-center text-[#FE7743] border border-white/10 group-hover:bg-[#FE7743]/10 transition-colors"><Zap className="w-5 h-5" /></div>
                            <div>
                        ...
                        <div className="flex items-center gap-3 text-gray-400 group">
                            <div className="w-10 h-10 rounded-full bg-white/5 flex items-center justify-center text-[#FE7743] border border-white/10 group-hover:bg-[#FE7743]/10 transition-colors"><Lock className="w-5 h-5" /></div>
                            <div>
                                <p className="text-white font-medium">Session Protection</p>
                                <p className="text-xs">Prevents unauthorized account takeover</p>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div className="w-full max-w-md bg-[#0A0A0A] border border-white/10 p-8 rounded-2xl shadow-2xl relative z-10">
                    ...
                    <div className="mb-8 text-center">
                        <div className="mx-auto w-16 h-16 bg-[#FE7743]/10 rounded-full flex items-center justify-center mb-6 border border-[#FE7743]/20 relative">
                            ...
                            {success ? <CheckCircle2 className="text-green-500 w-8 h-8 relative z-10" /> : <Key className="text-[#FE7743] w-8 h-8 relative z-10" />}
                        </div>
                    ...
                    <div className="space-y-8">
                        ...
                        <Button
                            ...
                            className={`w-full font-bold h-12 transition-all shadow-lg ${success
                                ? 'bg-green-500 hover:bg-green-600 text-black'
                                : 'bg-[#FE7743] hover:bg-[#FE7743]/90 text-black shadow-[0_0_15px_rgba(254,119,67,0.15)]'
                                }`}
                        >
                            {loading ? <Loader2 className="animate-spin" /> : success ? 'Access Granted' : 'Verify & Access Terminal'}
                        </Button>

                        <div className="text-center text-xs text-gray-500 font-mono">
                            Didn&apos;t receive code? <button className="hover:text-white transition-colors underline decoration-[#FE7743]">Resend in 30s</button>
                        </div>
                    </div>

                    <div className="mt-8 pt-6 border-t border-white/5 text-center">
                        <Link href="/login" className="text-xs text-gray-400 hover:text-white flex items-center justify-center gap-2 group">
...
export default function OTPPage() {
    return (
        <Suspense fallback={<div className="min-h-screen bg-[#050505] text-white flex items-center justify-center">Loading Security Gateway...</div>}>
            <OTPForm />
        </Suspense>
    );
}
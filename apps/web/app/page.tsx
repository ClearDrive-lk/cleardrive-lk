...
import { Search, ShieldCheck, Zap, Globe, TrendingUp, Car, Truck, Anchor, CheckCircle2, ArrowRight, Terminal } from 'lucide-react';
    ...
    <div className="min-h-screen bg-[#050505] text-white selection:bg-[#FE7743] selection:text-black font-sans flex flex-col">
        ...
        <div className="max-w-7xl mx-auto px-6 h-16 flex items-center justify-between">
            ...
            <div className="hidden md:flex gap-8 text-sm font-medium text-gray-400">
                <Link href="#" className="hover:text-white transition-colors flex items-center gap-2">
                    Auctions <Badge variant="outline" className="text-[10px] border-[#FE7743]/20 text-[#FE7743] h-4 px-1">LIVE</Badge>
                </Link>
                <Link href="#" className="hover:text-white transition-colors">Logistics</Link>
                <Link href="#" className="hover:text-white transition-colors">Tax Calculator</Link>
            </div>
            <div className="flex gap-4">
                <Link href="/login">
                    <Button variant="ghost" className="text-gray-400 hover:text-white hover:bg-white/5 font-mono">Sign In</Button>
                </Link>
                <Link href="/register">
                    <Button className="bg-[#FE7743] text-black hover:bg-[#FE7743]/90 font-bold">Get Access</Button>
                </Link>
            </div>
        </div>
      </nav>
      ...
      <section className="relative pt-20 pb-20 px-6 overflow-hidden flex-1 flex flex-col justify-center">
         ...
         <div className="relative z-10 max-w-5xl mx-auto text-center space-y-8">
            ...
            <h1 className="text-5xl md:text-8xl font-bold tracking-tighter text-white leading-[0.9]">
                IMPORT <span className="text-transparent bg-clip-text bg-gradient-to-r from-[#FE7743] to-orange-200">DIRECT.</span><br />
                PAY ZERO MARKUP.
            </h1>
            
            <p className="text-lg md:text-xl text-gray-400 max-w-2xl mx-auto leading-relaxed">
                The first <span className="text-white font-medium">Direct-to-Consumer</span> import terminal in Sri Lanka. 
                Access USS Tokyo, JAA, and CAI auctions in real-time. 
                <span className="block mt-2 text-gray-500 text-sm font-mono">
                    AUTOMATED CIF CALCULATION  INSTANT LC OPENING // CLEARING @ HAMBANTOTA
                </span>
            </p>
            ...
            <div className="max-w-2xl mx-auto mt-12 p-1 rounded-xl bg-gradient-to-b from-white/10 to-white/5 backdrop-blur-xl border border-white/10 shadow-2xl">
                <div className="relative flex items-center bg-[#0A0A0A] rounded-lg p-1.5">
                    ...
                    <div className="hidden md:flex items-center gap-2 pr-4">
                        <Badge variant="secondary" className="bg-[#1A1A1A] text-gray-500 hover:bg-[#1A1A1A]">VIN</Badge>
                        <Badge variant="secondary" className="bg-[#1A1A1A] text-gray-500 hover:bg-[#1A1A1A]">Lot #</Badge>
                    </div>
            ...
            <div className="pt-8 flex flex-wrap justify-center gap-6 text-sm text-gray-500 font-mono">
                <span className="flex items-center gap-2"><CheckCircle2 className="w-4 h-4 text-[#FE7743]" /> VERIFIED CONDITION SHEETS</span>
                <span className="flex items-center gap-2"><CheckCircle2 className="w-4 h-4 text-[#FE7743]" /> REAL-TIME ODOMETER CHECK</span>
                <span className="flex items-center gap-2"><CheckCircle2 className="w-4 h-4 text-[#FE7743]" /> JEVIC CERTIFIED</span>
            </div>
         </div>
      </section>
      ...
      <div className="border-b border-white/10 bg-[#0A0A0A]">
        <div className="max-w-7xl mx-auto grid grid-cols-2 md:grid-cols-4 divide-x divide-white/10">
            {[
                { label: "Auction Access", value: "USS, JAA, CAI", icon: Globe, sub: "Direct API Link" },
                { label: "Exchange Rate", value: "2.25 LKR/JPY", icon: TrendingUp, sub: "Live Bank Rate" },
                { label: "Active Listings", value: "45,240+", icon: Car, sub: "Updated 2s ago" },
                { label: "Clearance Time", value: "~14 Days", icon: Zap, sub: "Fast-Track available" },
            ].map((stat, i) => (
      ...
      <section className="py-24 px-6 bg-[#050505]">
          <div className="max-w-7xl mx-auto">
              <div className="flex flex-col md:flex-row justify-between items-end mb-12 gap-4">
                  <div>
                      <h2 className="text-3xl font-bold text-white tracking-tight">Browse Inventory</h2>
                      <p className="text-gray-400 mt-2 text-lg">Select your vehicle class to view live auction data.</p>
                  </div>
                  <Button variant="outline" className="border-white/10 hover:bg-white/5 hover:text-white gap-2">
              ...
              <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                  {[
                      { name: "Sedan / Saloon", count: "12,200+", icon: Car, bg: "from-blue-900/20 to-purple-900/20", code: "CAT-SDN" },
                      { name: "SUVs & 4x4", count: "8,150+", icon: Anchor, bg: "from-orange-900/20 to-red-900/20", code: "CAT-SUV" },
                      { name: "Commercial / Van", count: "2,300+", icon: Truck, bg: "from-green-900/20 to-emerald-900/20", code: "CAT-COM" },
                      { name: "Kei & Compact", count: "5,600+", icon: Zap, bg: "from-yellow-900/20 to-orange-900/20", code: "CAT-KEI" },
                  ].map((cat, i) => (
                      <div key={i} className={`group relative h-72 rounded-xl border border-white/10 bg-gradient-to-br ${cat.bg} p-6 flex flex-col justify-between overflow-hidden hover:border-[#FE7743]/50 transition-all cursor-pointer`}>
                          <div className="absolute inset-0 bg-black/40 group-hover:bg-transparent transition-colors duration-500" />
                          
                          <div className="relative z-10 flex justify-between items-start">
                              <Badge variant="outline" className="border-white/20 text-white/50 font-mono text-[10px]">{cat.code}</Badge>
                              <cat.icon className="w-8 h-8 text-white/40 group-hover:text-white transition-colors" />
                          </div>

                          <div className="relative z-10">
                              <h3 className="text-2xl font-bold text-white group-hover:translate-x-1 transition-transform">{cat.name}</h3>

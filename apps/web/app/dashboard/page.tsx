...
import { LayoutDashboard, ShoppingCart, FileText, Settings, LogOut, Bell, User } from 'lucide-react';
    ...
    <div className="min-h-screen bg-[#050505] text-white flex font-sans">
      ...
      <aside className="w-64 border-r border-white/10 bg-[#0A0A0A] hidden md:flex flex-col">
        <div className="h-16 flex items-center px-6 border-b border-white/10">
            <span className="font-bold text-xl tracking-tighter">CD<span className="text-[#FE7743]">.lk</span></span>
        </div>
        ...
        <div className="flex-1 py-6 px-3 space-y-1">
            {[
                ...
                <button key={i} className={`w-full flex items-center gap-3 px-3 py-2 rounded-md text-sm font-medium transition-colors ${item.active ? 'bg-[#FE7743]/10 text-[#FE7743]' : 'text-gray-400 hover:text-white hover:bg-white/5'}`}>
      ...
      <div className="flex-1 flex flex-col">
        ...
        <header className="h-16 border-b border-white/10 bg-[#050505]/80 backdrop-blur flex items-center justify-between px-6 sticky top-0 z-40">
            <div className="text-sm text-gray-400 font-mono">TERMINAL &gt; DASHBOARD</div>
             ...
             <div className="relative z-10 max-w-6xl mx-auto space-y-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    {[
                        ...
                        { title: "Est. Cost Calculator", value: "JPY/LKR", sub: "2.25 Live Rate" },
                        { title: "Account Status", value: "Active", sub: "Level 1 Verified", color: "text-green-500" },
                    ].map((card, i) => (
                        <Card key={i} className="bg-[#0A0A0A]/50 border-white/10 backdrop-blur">
                ...
                <div className="rounded-xl border border-dashed border-white/10 bg-white/5 h-64 flex flex-col items-center justify-center text-gray-500 gap-4">
                    ...
                    <Button variant="outline" className="border-white/10 hover:bg-white/5 hover:text-white">Browse Auction House</Button>
                </div>
             </div>

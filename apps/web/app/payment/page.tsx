...
function PaymentForm() {
  ...
  const handlePayment = async () => {
    ...
    try {
      const initiateResponse = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/v1/payments/initiate`, {
        method: 'POST',
      ...
      const urlResponse = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/v1/payments/generate-url`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
  ...
  return (
    <Card className="max-w-md mx-auto bg-slate-900 border-slate-800 shadow-2xl">
      <CardHeader className="text-center">
        ...
        <CardDescription className="text-slate-400">Order ID: {orderId}</CardDescription>
      </CardHeader>

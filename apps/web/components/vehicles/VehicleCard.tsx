      ...
      <div className="relative h-48 w-full overflow-hidden bg-gray-900 group-hover:scale-105 transition-transform duration-500">
        ...
        <div className="absolute top-2 left-2 flex gap-2">
          ...
          {vehicle.condition === 'New' ? (
            <Badge className="bg-green-500/20 text-green-400 font-bold border-green-500/50">NEW</Badge>
          ) : (
            <Badge className="bg-gray-500/20 text-gray-400 font-bold border-gray-500/50">USED</Badge>
          )}
        </div>
        <div className="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-black/90 to-transparent p-3">
          <div className="flex items-center gap-1 text-[#FE7743] text-xs font-mono">
            <Timer className="w-3 h-3" />
            <span>Ends on {new Date(vehicle.endTime).toLocaleDateString()}</span>
          </div>
        </div>
      </div>
      ...
      <CardContent className="p-4 space-y-3">
        ...
        <div className="space-y-1">
          ...
          <div className="text-right">
            <span className="text-[10px] text-gray-500">Est. Duty: {formatLKR(estDuty)}</span>
          </div>
        </div>
      </CardContent>

...
def generate_payhere_hash(
    merchant_id: str, order_id: str, amount: Decimal, currency: str, merchant_secret: str
) -> str:
    """
...
@router.post(
    "/initiate", response_model=PaymentInitiateResponse, status_code=status.HTTP_201_CREATED
)
async def initiate_payment(request: PaymentInitiateRequest, db: Session = Depends(get_db)):
    ...
    if request.order_id.startswith("test-"):
        ...
        if not order:
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND, detail=f"Order {request.order_id} not found"
            )

        # Check if order already has a pending payment (idempotency check)
        existing_payment = (
            db.query(Payment)
            .filter(Payment.order_id == request.order_id, Payment.status == PaymentStatus.PENDING)
            .first()
        )
...
async def generate_payment_url(request: PaymentUrlRequest, db: Session = Depends(get_db)):
    """
    ...
    if not payment:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail=f"Payment {request.payment_id} not found"
        )

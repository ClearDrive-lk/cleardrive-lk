...
class DataDeletionService:
    ...
    @staticmethod
    def anonymize_order_data(user: User, db: Session) -> int:
        orders = db.query(Order).filter(Order.user_id == user.id).all()
        scrubbed = 0

        for order in orders:
            encrypted_placeholder = encrypt_field("Deleted per GDPR request")
            order.shipping_address = encrypted_placeholder or "Deleted per GDPR request"
    ...
    async def process_deletion(
        *,
        ...
        try:
            ...
            DataDeletionService.anonymize_order_data(user, db)

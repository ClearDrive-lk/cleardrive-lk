...
async def send_test_email(
    ...
    **Story**: CD-120 - Email testing
    ...
    html_content = template.render(
        name=current_admin.name or "Admin", frontend_url="http://localhost:3000"
    )

    # Send test email
    success = await send_email(
        to_email=current_admin.email, subject="ClearDrive Email Test", html_content=html_content
    )

    if success:
        return {"message": "Test email sent successfully", "to_email": current_admin.email}
    else:
        return {"message": "Test email failed to send", "to_email": current_admin.email}


@router.post("/queue-test-email")
async def queue_test_email(current_admin: User = Depends(get_current_admin)):
    """

...
async def assign_exporter_to_order(
    ...
    **Returns:**
    ...
    print(f"\n{'='*70}")
    print(f"📦 EXPORTER ASSIGNMENT STARTED")
    ...
    # ===============================================================
    existing_shipment = db.query(ShipmentDetails).filter(
    ...
    shipment = ShipmentDetails(
        order_id=order.id,
        assigned_exporter_id=exporter.id
    )
    ...
    print(f"\n{'='*70}")
    print(f"🎉 EXPORTER ASSIGNMENT COMPLETED")
...
async def get_shipment_details(
    ...
    **Permissions**: Admin only
    ...
    shipment = db.query(ShipmentDetails).filter(
        ShipmentDetails.id == shipment_id
    ).first()

...
from app.modules.finance.schemas import (
    InsuranceQuoteRequest, InsuranceApproveRequest,
    InsuranceRejectRequest, InsuranceResponse
)
...
async def request_insurance_quote(
    ...
    **Story**: CD-33.5
    ...
    existing = db.query(VehicleInsurance).filter(
        VehicleInsurance.order_id == request.order_id
    ).first()
...
async def get_pending_insurance_quotes(
    ...
    quotes = db.query(VehicleInsurance).filter(
        VehicleInsurance.status == InsuranceStatus.PENDING
    ).all()

...
class LetterOfCredit(Base, UUIDMixin, TimestampMixin):
    ...
    # LC Details
    lc_number: Mapped[str | None] = mapped_column(
...
class VehicleFinance(Base, UUIDMixin, TimestampMixin):
    ...
    # Foreign Keys
    order_id: Mapped[PyUUID] = mapped_column(
        GUID(), ForeignKey("orders.id", ondelete="CASCADE"), nullable=False, unique=True
    )
    user_id: Mapped[PyUUID] = mapped_column(
        GUID(), ForeignKey("users.id", ondelete="CASCADE"), nullable=False
    )
    ...
    status: Mapped[FinanceStatus] = mapped_column(
        SQLEnum(FinanceStatus), default=FinanceStatus.PENDING
    )
...
class VehicleInsurance(Base, UUIDMixin, TimestampMixin):
    ...
    status: Mapped[InsuranceStatus] = mapped_column(
        SQLEnum(InsuranceStatus), default=InsuranceStatus.PENDING
    )

...
from app.modules.finance.schemas import (
    LCCreateRequest, LCApproveRequest, LCRejectRequest, LCResponse
)
...
async def request_letter_of_credit(
    ...
    **Access**: Customer (own orders)
    ...
    existing_lc = db.query(LetterOfCredit).filter(
        LetterOfCredit.order_id == request.order_id
    ).first()
...
async def get_pending_lc_requests(
    ...
    **Story**: CD-33.7
    ...
    lcs = db.query(LetterOfCredit).filter(
        LetterOfCredit.status == LCStatus.PENDING
    ).all()
...
async def get_my_lc_requests(
    ...
    lcs = db.query(LetterOfCredit).filter(
        LetterOfCredit.user_id == current_user.id
    ).all()

...
from app.modules.finance.schemas import (
    FinanceApplicationRequest, FinanceApproveRequest,
    FinanceRejectRequest, FinanceResponse
)
...
async def apply_for_finance(
    ...
    **Process**:
    ...
    existing = db.query(VehicleFinance).filter(
        VehicleFinance.order_id == request.order_id
    ).first()
...
async def approve_finance(
    ...
    **Story**: CD-33.4
    ...
    finance = db.query(VehicleFinance).filter(
        VehicleFinance.id == finance_id
    ).first()
    ...
    if monthly_rate > 0:
        monthly_payment = float(finance.loan_amount) * monthly_rate * (1 + monthly_rate)**n / ((1 + monthly_rate)**n - 1)
    else:
...
async def get_pending_finance_applications(
    ...
    applications = db.query(VehicleFinance).filter(
        VehicleFinance.status == FinanceStatus.PENDING
    ).all()
